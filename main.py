...
def get_county_attributes(svg_file_name):
    ...
    for path in doc.getElementsByTagName('path'):
        ...
        boundary=path.getAttribute('d')
        attribue_list.append((FIPS_code,boundary))
    doc.unlink()
    return attribue_list
...
def get_boundary_coordinates(boundary_data):
    ...
    #new_data=boundary_data.replace('M',' ')
    boundary_data=boundary_data.replace('L','M')
    new_split_data=boundary_data.split('M')[1:]
    coordinates_list=[]
    for pair in new_split_data:
        split=pair.split(",")
        coordinates_list.append((float(split[0]),float(split[1])))
